...
function setupUI() {
    ...
    if (titleApi) titleApi.textContent = settings.name || "Laurence API";
    if (descApi) descApi.textContent = settings.description || "Interactive API documentation with real-time testing";
    if (footer) footer.textContent = `© ${new Date().getFullYear()} ${settings.creator || "Laurence"} - ${settings.name || "Laurence API"}`;
